...
type RunnerOptions struct {
	...
	// on every check interval and return (exit) when signatures are valid (so that the
	// next orbit restart everything can be initialized properly).
	//
		...
		case <-ticker.C:
			...
			if r.opt.SignaturesExpiredAtStartup {
				if err := r.updater.UpdateMetadata(); err != nil {
					log.Debug().Msg("update metadata failed: %s")
				} else {
					// When UpdateMetadata returns no error it means signatures are valid.
					log.Info().Msg("expired metadata has been updated successfully, exiting")
					return nil
				}
			}
