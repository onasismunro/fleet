...
func main() {
	...
	app.Action = func(c *cli.Context) error {
		...
		if !c.Bool("disable-updates") || c.Bool("dev-mode") {
			...
			signaturesExpiredAtStartup := false
			if err := updater.UpdateMetadata(); err != nil {
				log.Info().Err(err).Msg("update metadata, using saved metadata")
				// When the root signature is expired, updater.UpdateMetadata fails with an expiration error.
				signaturesExpiredAtStartup = update.IsExpiredErr(err)
				// When the targets or snapshot signature is expired, updater.UpdateMetadata does not fail
				// but updater.Lookup does fail with an expiration error.
				if !signaturesExpiredAtStartup {
					if _, err := updater.Lookup("orbit"); err != nil && update.IsExpiredErr(err) {
						signaturesExpiredAtStartup = true
					}
				}
			}
...
func getFleetdComponentPaths(
	...
	if err := updater.UpdateMetadata(); err != nil {
		...
		if update.IsExpiredErr(err) {
			log.Error().Err(err).Msg("expired metadata, using local targets")

			//
			// Attempt to get local paths for the targets.
			// (We trust what we have in local paths.)
			//
